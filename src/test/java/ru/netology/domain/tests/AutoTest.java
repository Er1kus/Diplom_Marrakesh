...
import ru.netology.domain.pages.RequestPage;
...
public class AutoTest {
    ...
    class SuccessWayPayment {
        ...
        void shouldGoWithValid() {
            ...
            requestPage.validCardNumberPay(cardInfo);
        }
        ...
        void shouldGoWithInvalid() {
            ...
            requestPage.invalidCardNumberPay(cardInfo);
        }
        ...
        void shouldGoWithValidCredit() {
            var requestPage = new RequestPage();
            var cardInfo = DataHelper.getCardInfo();
            requestPage.validCardNumberCreditPay(cardInfo);
        }
    }
    ...
    class CardNumberValidation {
        ...
        void shouldInputShortCardNumber() {
            ...
            requestPage.shortCardNumberPay(cardInfo);
        }

        @Test
        @Name("20-ти значный номер карты")
        void shouldInputLongCardNumber() {
            var requestPage = new RequestPage();
            var cardInfo = DataHelper.getLongCardNumber();
            requestPage.longCardNumberPay(cardInfo);
        }
        ...
        void shouldInputEmptyFields() {
            ...
            requestPage.emptyCardData(cardInfo);
        }
        ...
        void shouldInputRandomData() {
            ...
            requestPage.randomCardData(cardInfo);
        }

        @Test
        @Name("Номер карты из одной цыфры")
        void shouldInputOneDigitCardNumber() {
            var requestPage = new RequestPage();
            var cardInfo = DataHelper.getOneDigitCardNumber();
            requestPage.cardNumberOfOneDigit(cardInfo);
        }
        ...
        void shouldInputNullsInCardNumber() {
            ...
            requestPage.cardNumberOfNulls(cardInfo);
        }

        @Test
        @Name("Ввод буквенных символов в поле 'Номер катры'")
        void shouldInputLettersInCardNumber() {
            var requestPage = new RequestPage();
            var cardInfo = DataHelper.getCardNumberOfLetters();
            requestPage.cardNumberOfLetters(cardInfo);
        }

        @Test
        @Name("Ввод спецсимволов в поле 'Номер карты'")
        void shouldInputPunctuationInCardNumber() {
            var requestPage = new RequestPage();
            var cardInfo = DataHelper.getPunctuationMarksInCardNumber();
            requestPage.cardNumberOfPunctuationAndOtherMarks(cardInfo);
        }

        @Test
        @Name("Ввод пробелов в поле 'Номер карты'")
        void shouldInputWhitespacesInCardNumber() {
            var requestPage = new RequestPage();
            var cardInfo = DataHelper.getWhitespacesInCardNumber();
            requestPage.cardNumberOfWhitespaces(cardInfo);
        }
    }
